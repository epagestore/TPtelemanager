						...
						<tbody>
                           ...
                           <tr>
                                ...
                                <td><?php echo $transaction['order_id']?></td>
                              	<td><a href="form.html"><?php echo $transaction['milestone_description']?></a> <!--<span class="label label-info">Item Status</span>--><br /></td>
                               
								<td class="actions">
									<?php if($transaction['milestone_status']==6){?>
                                    	Complete
                                    <?php }else  if($transaction['milestone_status']==1){?>
                                    	Pending
                                    <?php }else  if($transaction['milestone_status']==5){?>
                                    	Processing
                                    <?php }else  if($transaction['milestone_status']==2){?>
                                    	Approved
                                    <?php }?>
								</td>
                                <td class="actions">
                                <?php if($transaction['milestone_status']==6){?>
                                	<a href="<?php echo base_url();?>index.php/transaction/releaseMilestonePayment/<?php echo $transaction['transaction_id']?>" class="btn btn-small btn-primary ">Release Payment</a>
                                <?php }else{?>
